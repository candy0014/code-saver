...
let knownTags = [], fileOpenSeq = 0, tagSuggestIndex = -1, modalTagSuggestIndex = -1, modalSelectedTags = [];
...
function refreshKnownTags(){
  knownTags=[...new Set(problems.flatMap(p=>p.tags||[]).map(t=>String(t).trim()).filter(Boolean))]
...
function tagCandidates(query='', excluded=[]){
  const q=String(query).trim().toLowerCase(), ex=new Set(excluded.map(x=>String(x).toLowerCase()));
  return knownTags.filter(t=>!ex.has(t.toLowerCase()) && (!q || t.toLowerCase().includes(q)))
    .sort((a,b)=>{
      if(!q) return a.localeCompare(b,'zh-CN',{sensitivity:'base'});
      const al=a.toLowerCase(), bl=b.toLowerCase();
      const ap=al.startsWith(q)?0:1, bp=bl.startsWith(q)?0:1;
      return ap-bp || al.indexOf(q)-bl.indexOf(q) || a.localeCompare(b,'zh-CN',{sensitivity:'base'});
    }).slice(0,24);
}
function paintTagSuggestions(input, box, excluded, activeIndex=0){
  const list=tagCandidates(input.value,excluded);
  if(!list.length){box.innerHTML='<div class="tag-suggestions-empty">没有匹配的已有标签</div>';box.classList.remove('hidden');return list}
  const q=input.value.trim().toLowerCase();
  box.innerHTML=list.map((t,i)=>{
    let label=esc(t);
    if(q){const low=t.toLowerCase(),pos=low.indexOf(q);if(pos>=0)label=esc(t.slice(0,pos))+`<span class="match">${esc(t.slice(pos,pos+q.length))}</span>`+esc(t.slice(pos+q.length));}
    return `<button type="button" class="tag-suggestion ${i===activeIndex?'active':''}" data-tag-i="${i}">${label}</button>`;
  }).join('');
  box.classList.remove('hidden');
  return list;
}
function closeTagSuggestions(box){box.classList.add('hidden');box.innerHTML=''}
function addKnownTag(tag){
  const v=String(tag).trim();if(!v)return;
  if(!knownTags.some(t=>t.toLowerCase()===v.toLowerCase())) knownTags.push(v),knownTags.sort((a,b)=>a.localeCompare(b,'zh-CN',{sensitivity:'base'}));
}

async function loadProblems(selectId=null){
  const x=await api('/api/problems'); problems=x.problems; refreshKnownTags(); renderProblemList();
...
function renderProblemList(){
  ...
  $('#problemList').innerHTML=list.map(p=>`<div class="problem-item ${current?.id===p.id?'active':''}" data-id="${esc(p.id)}"><div class="name">${esc(p.title)}</div><div class="meta"><span>${esc(p.id)}</span><span>${esc(p.time||'')}</span>${p.difficulty?`<span>${esc(p.difficulty)}</span>`:''}<span>${esc((p.tags||[]).slice(0,2).join(' · '))}</span></div></div>`).join('');
...
async function selectProblem(id){
  ...
  current=await api('/api/problems/'+encodeURIComponent(id)); resetFileEditor(); dirty=false;
...
function renderTags(){
  // The tag body is deliberately non-clickable. Only the small explicit ×
  // button can remove a tag, avoiding accidental deletion when selecting text
  // or clicking elsewhere on the chip.
  $('#tags').innerHTML=current.data.tags.map((t,i)=>`<span class="chip safe-tag-chip"><span class="chip-label">${esc(t)}</span><button type="button" class="chip-remove" data-tag-remove="${i}" aria-label="删除标签 ${esc(t)}" title="删除标签">×</button></span>`).join('');
  $$('#tags .chip-remove').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();current.data.tags.splice(+b.dataset.tagRemove,1);renderTags();markDirty()});
}
function addTag(value=null){
  const i=$('#tagInput'),v=String(value??i.value).trim();
  if(v&&!current.data.tags.includes(v)){current.data.tags.push(v);addKnownTag(v);renderTags();markDirty()}
  i.value='';tagSuggestIndex=-1;closeTagSuggestions($('#tagSuggestions'));
}
function bindCurrentTagSuggestionClicks(list){
  $('#tagSuggestions').querySelectorAll('[data-tag-i]').forEach(b=>b.onmousedown=e=>{e.preventDefault();const t=list[+b.dataset.tagI];if(t)addTag(t)});
}
function showCurrentTagSuggestions(){
  tagSuggestIndex=0;
  const list=paintTagSuggestions($('#tagInput'),$('#tagSuggestions'),current?.data?.tags||[],tagSuggestIndex);
  bindCurrentTagSuggestionClicks(list);
}
function handleCurrentTagKey(e){
  const box=$('#tagSuggestions'),visible=!box.classList.contains('hidden');
  if(e.key==='ArrowDown'||e.key==='ArrowUp'){
    e.preventDefault();const list=tagCandidates($('#tagInput').value,current?.data?.tags||[]);if(!list.length)return;
    tagSuggestIndex=visible?(tagSuggestIndex+(e.key==='ArrowDown'?1:-1)+list.length)%list.length:0;
    paintTagSuggestions($('#tagInput'),box,current?.data?.tags||[],tagSuggestIndex);bindCurrentTagSuggestionClicks(list);
  }else if(e.key==='Enter'){
    e.preventDefault();const list=tagCandidates($('#tagInput').value,current?.data?.tags||[]);
    if(visible&&list.length&&tagSuggestIndex>=0) addTag(list[Math.min(tagSuggestIndex,list.length-1)]); else addTag();
  }else if(e.key==='Escape'){closeTagSuggestions(box)}
}
function renderModalTagChips(){
  const box=$('#mTagChips');if(!box)return;
  box.innerHTML=modalSelectedTags.map((t,i)=>`<span class="chip safe-tag-chip"><span class="chip-label">${esc(t)}</span><button type="button" class="chip-remove" data-mtag-del="${i}" aria-label="删除标签 ${esc(t)}" title="删除标签">×</button></span>`).join('');
  box.querySelectorAll('.chip-remove[data-mtag-del]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();modalSelectedTags.splice(+b.dataset.mtagDel,1);renderModalTagChips();showModalTagSuggestions()});
}
function addModalTag(value){
  const input=$('#mTags'),v=String(value??input?.value??'').trim();if(!v)return;
  if(!modalSelectedTags.some(t=>t.toLowerCase()===v.toLowerCase())) modalSelectedTags.push(v),addKnownTag(v);
  if(input)input.value='';modalTagSuggestIndex=0;renderModalTagChips();showModalTagSuggestions();
}
function showModalTagSuggestions(){
  const input=$('#mTags'),box=$('#mTagSuggestions');if(!input||!box)return;
  const list=tagCandidates(input.value,modalSelectedTags);modalTagSuggestIndex=Math.max(0,Math.min(modalTagSuggestIndex,list.length-1));
  if(!list.length){box.innerHTML='<div class="tag-suggestions-empty">没有匹配的已有标签</div>';box.classList.remove('hidden');return list}
  box.innerHTML=list.map((t,i)=>`<button type="button" class="tag-suggestion ${i===modalTagSuggestIndex?'active':''}" data-tag-i="${i}">${esc(t)}</button>`).join('');box.classList.remove('hidden');
  box.querySelectorAll('[data-tag-i]').forEach(b=>b.onmousedown=e=>{e.preventDefault();addModalTag(list[+b.dataset.tagI])});return list;
}
function setupModalTagAutocomplete(){
  const input=$('#mTags'),box=$('#mTagSuggestions');if(!input||!box)return;
  modalSelectedTags=[];modalTagSuggestIndex=0;renderModalTagChips();
  input.onfocus=showModalTagSuggestions;input.oninput=()=>{modalTagSuggestIndex=0;showModalTagSuggestions()};
  input.onkeydown=e=>{
    const list=tagCandidates(input.value,modalSelectedTags);
    if(e.key==='ArrowDown'||e.key==='ArrowUp'){
      e.preventDefault();if(!list.length)return;modalTagSuggestIndex=(modalTagSuggestIndex+(e.key==='ArrowDown'?1:-1)+list.length)%list.length;showModalTagSuggestions();
    }else if(e.key==='Enter'){
      e.preventDefault();if(!box.classList.contains('hidden')&&list.length&&input.value.trim())addModalTag(list[Math.min(modalTagSuggestIndex,list.length-1)]);else addModalTag();
    }else if(e.key===','){e.preventDefault();addModalTag()}else if(e.key==='Escape')closeTagSuggestions(box)
  };
  input.onblur=()=>setTimeout(()=>closeTagSuggestions(box),120);
}
...
async function openFile(name){
  const pid=current?.id, seq=++fileOpenSeq;
  if(!pid)return;
  ...
  const x=await api(`/api/problems/${encodeURIComponent(pid)}/files/${encodeURIComponent(name)}`);
  if(seq!==fileOpenSeq || current?.id!==pid) return;
  currentFile=name;$('#fileName').textContent=name;$('#fileMeta').textContent='Ctrl+S 保存';$('#fileEditor').disabled=false;$('#fileEditor').value=x.content;$('#saveFileBtn').classList.remove('hidden');$('#deleteFileBtn').classList.remove('hidden');renderFiles();
}
...
async function newProblem(){
  const pending=modal(`<h2>新建题目</h2><div class="modal-grid"><label>题目 ID<input class="modal-input" id="mId" placeholder="QOJ12345" autofocus></label><label>展示名称（可选）<input class="modal-input" id="mTitle"></label><label>主链接题号<input class="modal-input" id="mLink" placeholder="留空则与 ID 相同"></label><label>难度<input class="modal-input" id="mDiff" type="number"></label><label>标签<div id="mTagChips" class="chips modal-tag-chips"></div><div class="modal-tag-wrap"><input class="modal-input" id="mTags" autocomplete="off" placeholder="输入一个标签，回车添加"><div id="mTagSuggestions" class="tag-suggestions hidden"></div></div></label><label><input id="mSol" type="checkbox"> 同时创建 sol.md</label></div>`,'创建');
  setupModalTagAutocomplete();
  const ok=await pending;if(!ok)return;
  try{const id=$('#mId').value.trim();const x=await api('/api/problems',{method:'POST',body:JSON.stringify({id,title:$('#mTitle').value,link:$('#mLink').value,difficulty:$('#mDiff').value,tags:[...modalSelectedTags,...($('#mTags').value.trim()?[ $('#mTags').value.trim() ]:[])],create_solution:$('#mSol').checked})});toast('题目 '+id+' 已创建');await loadProblems(x.id)}catch(e){toast(e.message,true)}
}
...
async function deleteFile(){if(!currentFile||!confirm(`确定删除 ${currentFile}？同时会移除指向它的 #Code/#Solution 链接。`))return;try{current=await api(`/api/problems/${encodeURIComponent(current.id)}/files/${encodeURIComponent(currentFile)}`,{method:'DELETE'});toast(currentFile+' 已删除');resetFileEditor();renderAll();activateTab('files')}catch(e){toast(e.message,true)}}
...
$('#search').oninput=renderProblemList;$('#newProblemBtn').onclick=newProblem;$('#renameBtn').onclick=renameProblem;$('#saveBtn').onclick=saveStructured;$('#saveConfBtn').onclick=saveRaw;$('#addTagBtn').onclick=()=>addTag();
$('#tagInput').onfocus=showCurrentTagSuggestions;$('#tagInput').oninput=showCurrentTagSuggestions;$('#tagInput').onkeydown=handleCurrentTagKey;$('#tagInput').onblur=()=>setTimeout(()=>closeTagSuggestions($('#tagSuggestions')),120);
$('#title').oninput=()=>markDirty();$('#difficulty').oninput=()=>markDirty();
